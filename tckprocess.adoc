...
wishing to use the MicroProfile Platform Compatible logo or
https://microprofile.io[microprofile.io] website for promotion. Any
request for certification from an organization not meeting the requirements
will be held until such time as the requirements are met. See the
https://microprofile.io/legal/trademark_guidelines/[MicroProfile Trademark Guidelines] for more
information on the complete steps to obtain  branding usage.

An approved certification request is a statement from the Specification
contributors you have met the intended TCK requirements and is just one of
the requirements for logo usage. Logos only apply to the platform specification.
...
via the specification's issue tracker using the label
...
use the MicroProfile Platform Compatible logo. Approval that the TCK
requirements have been met is a prerequisite for obtaining
MicroProfile Platform Compatible logo usage for profile
...
https://www.eclipse.org/legal/tck.php[Eclipse Foundation Technology Compatibility Kit License]. If use of the
MicroProfile Platform Compatible logo or trademark is being requested (currently only
applicable to platform or profile products), see the https://microprofile.io/legal/trademark_guidelines/[MicroProfile Trademark Guidelines].
